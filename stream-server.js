...
var STREAM_SECRET = process.argv[2] || 1234,
...
socketServer.on('connection', function(socket) {
	...
	socket.on('close', function(code, message){
		socketServer.clients.delete(socket);
		console.log( 'Disconnected WebSocket ('+socketServer.clients.size+' total)' );
	});

	socket.on('error', function(code, message){
		socketServer.clients.delete(socket);
		console.log( 'WebSocket meet error, code :' + code);
	});
});

socketServer.broadcast = function(data, opts) {
	for( var i of this.clients ) {
		if (i.readyState == 1) {
			i.send(data, opts);
		}else {
